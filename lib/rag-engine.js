...
Standalone Query:`;
  ...
  const hasCareerKeyword = /\b(skills|skill|experience|experiences|project|projects|internship|internships|certifications?|certificates?|awards?|achievements?|publications?|patents?|linkedin|github|portfolio|who am i|about me|who is the candidate|who is the author|candidate name|my name)\b/i.test(effectiveQuery);
  ...
  if (chunks && chunks.length > 0) {
    ...
    if (isMultiDocSummary && docSources.length > 1) {
      ...
      for (const src of docSources) {
        ...
        const topForDoc = await docStore.similaritySearch(effectiveQuery, 3);
      ...
      retrievedDocs = await store.similaritySearch(effectiveQuery, 5);
  ...
  const mentionsComparison =
    /\b(compare|comparison|versus|vs|difference between|how do my|match with|stack against|in demand|industry trend|market trend|latest trends|market demand)\b/i.test(effectiveQuery);

  const isGeneralConceptualQuery =
    /\b(difference between|what is the difference|how does .+ work|explain .+ concept|overview of|compare .+ and)\b/i.test(effectiveQuery) &&
    !hasPersonalPronoun &&
    !hasDocKeyword;

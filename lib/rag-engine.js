...
export async function executeRagPipeline(
  ...
  // - CORRECT: Query is about the resume / candidate / attached docs -> Use internal docs only.
  // - AMBIGUOUS (Hybrid Web Blend): Query compares resume with live external market/trends -> Blend both if web enabled.
  // - INCORRECT (Web Fallback): Query is about external topic / tech not in resume -> PURE web search.
  ...
  if (retrievedDocs.length === 0) {
    ...
    triggerWebSearch = false;
    useInternalDocs = true;
  } else if (isExplicitExternalTopic || topScore < 0.25) {
    cragVerdict = 'INCORRECT (Web Fallback)';
    triggerWebSearch = true;
    useInternalDocs = false;
  } else {

"""
Phase 7 — Claim / Evidence Verification Graph
=============================================
Adds explicit claim extraction and evidence verification after the 
Self-RAG loop but before Confidence Scoring. This provides the 
Confidence Scorer with objective proof of hallucinations (or lack thereof).
...
::
    ...
    generate_from_context 
     ...
     is_sup (Self-RAG check)
       ...
       └── extract_and_verify_claims  <-- NEW IN PHASE 7
             │
             ▼
         score_confidence
             │
...
from acsrag.nodes.claim_verifier import extract_and_verify_claims  # NEW IN PHASE 7
from acsrag.nodes.confidence_scorer import score_confidence, route_after_confidence

def route_after_issup_phase7(state: ACSRAGState) -> str:
...
def build_phase7_graph(pdf_paths: List[str | Path]):
    """Build the Phase 7 Claim Verification graph."""

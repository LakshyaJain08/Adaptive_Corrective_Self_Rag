"""
Phase 5 — Intent Classification & Adaptive Routing Graph
========================================================
Adds an Intent Classifier at the very beginning of the pipeline.
The router uses this intent to decide whether to fire Vector, BM25, Web, 
or a combination of them, saving time and tokens when all aren't needed.

Graph Flow
----------
::
    START → classify_intent → decide_retrieval
                                ├── NO  → generate_direct → END
                                └── YES → generate_multi_queries
                                            │
                                            ▼
                                   route_retrieval (ADAPTIVE)
                                     ├── vector_search
                                     ├── bm25_search (if factual/comparative/multi-hop)
                                     └── web_search (if temporal/out-of-domain)
                                            │
                                            ▼
                                      rrf_fusion
                                            │
...
def build_phase5_graph(pdf_paths: List[str | Path]):
    """Build the Phase 5 Intent & Routing graph."""
    ...
    # ADAPTIVE routing based on intent (vector, bm25, web)

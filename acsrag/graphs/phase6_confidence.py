"""
Phase 6 — Explicit Confidence Scoring Graph
===========================================
Adds quantitative confidence scoring at the end of the Self-RAG loop.
If the overall confidence score is below threshold, it routes back for 
another round of retrieval (Iterative Retrieval).
     ...
     is_sup (Self-RAG check)
       ...
       └── score_confidence
         ...
         route_after_confidence
             ├── OVER_THRESHOLD  → accept_answer → END
             └── UNDER_THRESHOLD → rewrite_question → generate_multi_queries (Iterative Retrieval loop)
"""
...
from acsrag.nodes.confidence_scorer import score_confidence, route_after_confidence # NEW IN PHASE 6

def route_after_issup_modified(state: ACSRAGState) -> str:
    """Modified route_after_issup to point to score_confidence instead of is_use."""
...
def build_phase6_graph(pdf_paths: List[str | Path]):
    """Build the Phase 6 Confidence Scoring graph."""
    ...
    # NEW: Confidence Scorer (replaces is_use)
    ...
    # Modified Reflection loop using the new router
    g.add_conditional_edges(
        "is_sup",
        route_after_issup_modified,
        {"score_confidence": "score_confidence", "revise_answer": "revise_answer"},
    )

"""
Phase 4 — Dynamic Context Compression Graph
===========================================
Replaces the basic `refine` node with `compress_context`. This ensures the 
generator only receives the most highly-relevant passages that fit within a 
strict token budget, thereby saving LLM costs and reducing hallucinations.
                  ...
                  └── web_search_parallel
                    ...
                    rrf_fusion
                     ...
                     rerank_docs
                          ...
                          ▼
                    eval_each_doc (CRAG)
                      ├── CORRECT   → compress_context ...
                      ├── AMBIGUOUS → rewrite_query → web_search_fallback → compress_context ...
                      └── INCORRECT → rewrite_query → web_search_fallback → compress_context ...
                          │
                          ▼
                  generate_from_context (Self-RAG loop follows)
"""
...
from acsrag.nodes.context_compressor import make_compressor_node  # NEW IN PHASE 4
...
def build_phase4_graph(pdf_paths: List[str | Path]):
    """Build the Phase 4 Context Compression graph."""
    ...
    # We share the cross-encoder for both reranking and compression!
    ...
    # NEW: compress_context instead of refine
    ...
    # CRAG fallback -> route to compress_context
    g.add_conditional_edges(
        "eval_each_doc",
        route_after_eval,
        {
            "refine": "compress_context",  # Phase 4 update
